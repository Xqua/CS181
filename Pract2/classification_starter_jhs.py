...
def syscall_name_entropy(tree):
    N = total_syscall_name(tree)
    c = syscall_name_counter(tree)
    entropy = Counter()
    in_all_section = False
    for el in tree.iter():
        # ignore everything outside the "all_section" element
        if el.tag == "all_section" and not in_all_section:
            ...
            for name in syscallname:
                p.append(float(c[name]/N))
            entropy[el.tag]=sum(p*np.log(p))
    return entropy
